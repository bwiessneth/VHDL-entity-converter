...
{
	MSG(GROUP::DEBUG, DEBUG::FUNCTIONCALL) << "LaTeX::LaTeX(VHDLEntity sourceEntity)";
...
std::string LaTeX::Polarity(int i)
{
...
std::string LaTeX::Direction(int i)
{
...
void LaTeX::getTable()
{
	if (cfg.getBool("LaTeX.addTable"))
	{
		...
		if (cfg.getBool("LaTeX.caption"))
			mOutputFile << "\\caption{" << replaceStr(mSourceEntity.getEntityName(), "_", "\\_") << "}" << std::endl;

		if (cfg.getBool("LaTeX.label"))
			mOutputFile << "\\label{ref:" << mSourceEntity.getEntityName() << "}" << std::endl;
	}
...
void LaTeX::getGenerics()
{
	...
	{
		writeCell(colNames::GenericName, mSourceEntity.getGenericName(i), " ", " ", " ");
		mOutputFile << "&";
		...
		if (cfg.getBool("Table.showArrayLength") && mSourceEntity.getGenericIsVector(i))
		{
			if (cfg.getBool("Table.arrayNotation"))
				tmpStr += "[" + std::to_string(mSourceEntity.getGenericVectorLength(i)) + "]";
			else
				tmpStr += "[" + std::to_string(mSourceEntity.getGenericVectorStart(i)) + ":" + std::to_string(mSourceEntity.getGenericVectorEnd(i)) + "]";
		}
		writeCell(colNames::GenericType, tmpStr, " ", " ", " ");
			
			
		mOutputFile << "&";
		writeCell(colNames::DefaultValue, mSourceEntity.getGenericDefaultValue(i), " ", " ", " ");
		mOutputFile << "\\\\";
...
void LaTeX::getHeaderGenerics()
{
	...
	if (cfg.getBool("Table.boldHeadings"))
		writeCell(colNames::Name, cfg.getString("Table.GenericName"), "\\textbf{", "} ", " ");
	else
	...
	if (cfg.getBool("Table.boldHeadings"))
		writeCell(colNames::Type, cfg.getString("Table.GenericType"), " \\textbf{", "} ", " ");
	else
		writeCell(colNames::Type, cfg.getString("Table.GenericType"), " ", " ", " ");

	mOutputFile << "&";

	if (cfg.getBool("Table.boldHeadings"))
		writeCell(colNames::Direction, cfg.getString("Table.Direction_heading"), " \\textbf{", "} ", " ");
	else
		writeCell(colNames::Direction, cfg.getString("Table.Direction_heading"), " ", " ", " ");
...
void LaTeX::getHeaderPorts()
{
	...
	if (cfg.getBool("Table.boldHeadings"))
		writeCell(colNames::Name, cfg.getString("Table.Name_heading"), "\\textbf{", "} ", " ");
	else
	...
	if ((cfg.getBool("Table.exportType")) && !cfg.getBool("Table.combineNameAndType"))
	{
		...
		if (cfg.getBool("Table.boldHeadings"))
			writeCell(colNames::Type, cfg.getString("Table.Type_heading"), " \\textbf{", "} ", " ");
		else
			writeCell(colNames::Type, cfg.getString("Table.Type_heading"), " ", " ", " ");
	}
	...
	if (cfg.getBool("Table.exportPolarity"))
	{
		...
		if (cfg.getBool("Table.boldHeadings"))
			writeCell(colNames::Polarity, cfg.getString("Table.Polarity_heading"), " \\textbf{", "} ", " ");
		else
			writeCell(colNames::Polarity, cfg.getString("Table.Polarity_heading"), " ", " ", " ");
	}
	...
	if (cfg.getBool("Table.exportDescription"))
	{
		...
		if (cfg.getBool("Table.boldHeadings"))
			writeCell(colNames::Description, cfg.getString("Table.Description_heading"), " \\textbf{", "} ", " ");
		else
			writeCell(colNames::Description, cfg.getString("Table.Description_heading"), " ", " ", " ");
	}
	...
	if (cfg.getBool("Table.exportBlank1"))
	{
		...
		if (cfg.getBool("Table.boldHeadings"))
			writeCell(colNames::Blank, cfg.getString("Table.Blank1_heading"), " \\textbf{", "} ", " ");
		else
			writeCell(colNames::Blank, cfg.getString("Table.Blank1_heading"), " ", " ", " ");
	}
	...
	if (cfg.getBool("Table.exportBlank2"))
	{
		...
		if (cfg.getBool("Table.boldHeadings"))
			writeCell(colNames::Blank, cfg.getString("Table.Blank2_heading"), " \\textbf{", "} ", " ");
		else
			writeCell(colNames::Blank, cfg.getString("Table.Blank2_heading"), " ", " ", " ");
	}
	mOutputFile << "\\\\" << std::endl;
	mOutputFile << "\\hline" << std::endl;
}
	...
	for (int i = 0; i < noPorts; i++)
	{
		...
		if (cfg.getBool("Table.showArrayLength") && mSourceEntity.isVector(i) && cfg.getBool("Table.combineNameAndType"))
		{
			if (cfg.getBool("Table.arrayNotation"))
				...
				tmpStr += "[" + mSourceEntity.getVectorStartStr(i) + ":" + mSourceEntity.getVectorEndStr(i) + "]";
		}
...
// Get the maximum text lengths for each column in order to align the table content
void LaTeX::getMaxTextLengths()
{
	...
	for (int i = 0; i < noPorts; i++)
	{
		colWidthName = max(colWidthName, replaceStr(mSourceEntity.getPortName(i), "_", "\\_").length());
		colWidthType = max(colWidthType, replaceStr(mSourceEntity.getPortTypeStr(i), "_", "\\_").length());
		...
		if (mSourceEntity.isVector(i))
		{
			if (cfg.getBool("Table.showArrayLength"))
			{
				...
				{
					if (cfg.getBool("Table.arrayNotation"))
						colWidthName = max(colWidthName, replaceStr(mSourceEntity.getPortName(i),"_", "\\_").length() + mSourceEntity.getVectorStr(i).length() + 2);
					else
						colWidthName = max(colWidthName, replaceStr(mSourceEntity.getPortName(i), "_", "\\_").length() + mSourceEntity.getVectorStartStr(i).length() + mSourceEntity.getVectorEndStr(i).length() + 3);
				}
				else if (cfg.getBool("Table.exportType"))
				{
					if (cfg.getBool("Table.arrayNotation"))
						colWidthType = max(colWidthType, replaceStr(mSourceEntity.getPortTypeStr(i), "_", "\\_").length() + mSourceEntity.getVectorStr(i).length() + 2);
					else
						colWidthType = max(colWidthType, replaceStr(mSourceEntity.getPortTypeStr(i), "_", "\\_").length() + mSourceEntity.getVectorStartStr(i).length() + mSourceEntity.getVectorEndStr(i).length() + 3);
				}
			}
		}
	}
	...
	colWidthName = max(colWidthName, cfg.getString("Table.Name_heading").length() + formattingCorrection);
	colWidthType = max(colWidthType, cfg.getString("Table.Type_heading").length() + formattingCorrection);
	colWidthDirection = max(colWidthDirection, cfg.getString("Table.Direction_heading").length() + formattingCorrection);
	colWidthPolarity = max(colWidthPolarity, cfg.getString("Table.Polarity_heading").length() + formattingCorrection);
	colWidthDescription = max(colWidthDescription, cfg.getString("Table.Description_heading").length() + formattingCorrection);
	colWidthBlank = max(colWidthBlank, cfg.getString("Table.Blank1_heading").length() + formattingCorrection);
	colWidthBlank = max(colWidthBlank, cfg.getString("Table.Blank2_heading").length() + formattingCorrection);
	colWidthGenericName = max(colWidthGenericName, cfg.getString("Table.Name_heading").length() + formattingCorrection);
	colWidthGenericType = max(colWidthGenericType, cfg.getString("Table.Type_heading").length() + formattingCorrection);
	colWidthDefaultValue = max(colWidthDefaultValue, std::string("Default value").length() + formattingCorrection);
...
void LaTeX::writeCell(colNames column, std::string text, std::string prefix, std::string suffix, std::string fillChar, bool formatting)
{

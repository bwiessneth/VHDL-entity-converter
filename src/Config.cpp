...
// Initialize a 2-dim string array which includes all configuration keys and their default value
std::string Config::configurationKeys[][2] = {
	...
	{ "PATH.soffice", "C:\\Program Files (x86)\\LibreOffice 4\\program\\soffice.exe" },
...
void Config::setDefaultConfigValues()
{
	MSG(GROUP::DEBUG) << "Config::setDefaultConfigValues()";

	// Get the number of elements of keys to create
	int numberOfElements = sizeof(configurationKeys) / sizeof(configurationKeys[0]);

	// Loop through all keys
	for (int i = 0; i < numberOfElements; i++)
	{
		// Insert new entries into the configuratin map using the current key and the default value
		configMap.insert(dataPair(configurationKeys[i][0], configurationKeys[i][1]));
	}
...
std::string Config::getString(std::string key)
{
	...
	{
		// Requested key doesn't exist in configMap. Display an Error and return an empty string
		std::cout << "Unknown key: " << key << " Returning >(string) \"\"<" << std::endl;
		return "";
	}
	else
	{
...
bool Config::getBool(std::string key)
{
	...
	if (mIt == configMap.end())
	{
		// Requested key doesn't exist in configMap. Display an Error and return false
		std::cout << "Unknown key: " << key << " Returning >(bool) false<" << std::endl;
		return false;
	}
	else
	{
		// If the key exists get its value and check if its either "1" or "true" and return true
...
double Config::getDouble(std::string key)
{
	...
	if (mIt == configMap.end())
	{
		...
		std::cout << "Unknown key: " << key << " Returning >(double) 0<" << std::endl;
		return 0;
	}
	else
	{
...
float Config::getFloat(std::string key)
{
	...
	{
		// Requested key doesn't exist in configMap. Display an Error and return 0
		std::cout << "Unknown key: " << key << " Returning >(float) 0<" << std::endl;
		return 0;
	}
	else
	{
...
std::string Config::readValueString(std::string key)
{
	std::string line = "";
	...
	while (getline(cF, line))
	{
		...
		if (kPos != -1)
		{
			...
			std::string potComment = line.substr(0, kPos);	// aquire string between keyword and line break		
			// Check if substring contains comment indicating char	
			if (potComment.find("#") == std::string::npos)		// found no comment in string
			{

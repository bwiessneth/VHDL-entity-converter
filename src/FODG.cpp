...
{
	MSG(GROUP::DEBUG, DEBUG::FUNCTIONCALL) << "FODG::FODG(VHDLEntity sourceEntity)";
...
void FODG::initValues()
{
	MSG(GROUP::DEBUG, DEBUG::FUNCTIONCALL) << "FODG::initValues(VHDLEntity mSourceEntity)";
...
void FODG::translate()
{
	...
	// svg:y = (portY / (entityHeight/2) * resolution * direction) = (1.5cm / 4cm = 0.375cm) * 5 * -1 = -1.875cm	
	double relX = 0;
	double relY = 0;
	...
	if (genericWidth > 0)
	{
		...
		if (vectorTextLenghtOut > genericWidth)
		{
			...
			if (mSourceEntity.getResetPort() != -1)
				...
				sWriter.FODGHeader(entityWidth + 2 * horizontal_Margin + vectorTextLenghtIn + vectorTextLenghtOut, entityHeight + 1 + genericHeight + 2 * vertical_Margin);
			else
				// Entity has no reset port
				sWriter.FODGHeader(entityWidth + 2 * horizontal_Margin + vectorTextLenghtIn + vectorTextLenghtOut, entityHeight + genericHeight + 2 * vertical_Margin);
		}
		else
		{
			...
			{
				// Entity has a reset port
				sWriter.FODGHeader(entityWidth + 2 * horizontal_Margin + vectorTextLenghtIn + genericWidth, entityHeight + 1 + genericHeight + 2 * vertical_Margin);
			}
			else
			{
				// Entity has no reset port
				sWriter.FODGHeader(entityWidth + 2 * horizontal_Margin + vectorTextLenghtIn + genericWidth, entityHeight + genericHeight + 2 * vertical_Margin);
			}
		}
	}
	else
	{
		...
		if (mSourceEntity.getResetPort() != -1)
			// Entity has a reset port
			sWriter.FODGHeader(entityWidth + 2 * horizontal_Margin + vectorTextLenghtIn + vectorTextLenghtOut, entityHeight + 2 + 2 * vertical_Margin);
		else
			// Entity has no reset port
			sWriter.FODGHeader(entityWidth + 2 * horizontal_Margin + vectorTextLenghtIn + vectorTextLenghtOut, entityHeight + 1 + 2 * vertical_Margin);
	}
	...
	if (mSourceEntity.getNumberOfGenerics() > 0)
	{
		...
		// Add generic box
		sWriter.customShape(currentX + entityWidth, currentY, genericWidth, genericHeight, "", "generics", "generics");

		// Loop through all generics
		for (int i = 0; i < mSourceEntity.getNumberOfGenerics(); i++)
		{
			// Add shape for each generic
			sWriter.customShape(currentX + entityWidth, currentY + i*0.5, genericWidth, 0.5, mSourceEntity.getGenericStr(i), "genericSignal", "generics");
		}
	...
	if (mSourceEntity.getEntityLabel().length() != std::string::npos)
		sWriter.customShape(currentX, currentY, entityWidth, 1, mSourceEntity.getEntityLabel(), "label", "tlabel");

	// Draw enitity box
	sWriter.customShape(currentX, currentY + 1, entityWidth, entityHeight, mSourceEntity.getEntityName(), "entity", "tentity");
	...
	for (int i = 0; i < noPorts; i++){
		...
		if (mSourceEntity.getPortdirection(i) == EntityPort::IN)
		{
			...
			else
			{
				...
				sWriter.customShape(currentX, currentY + noInputs + 1, portWidth, 1, mSourceEntity.getPortName(i), "port", "tlsport");
				sWriter.line(currentX - 1, currentY + noInputs + 1.5, currentX, currentY + noInputs + 1.5, "connector");
				if (mSourceEntity.getLOWActive(i) == true)
					sWriter.circle(currentX - 0.125, currentY + noInputs + 1.5, 0.25, "connector");
		...
		else if (mSourceEntity.getPortdirection(i) == EntityPort::OUT)
		{
			...
			// Offset for connector line = 1.5
			sWriter.customShape(currentX + portWidth, currentY + noOutputs + 1, portWidth, 1, mSourceEntity.getPortName(i), "port", "trsport");
			sWriter.line(currentX + entityWidth, currentY + noOutputs + 1.5, currentX + entityWidth + 1, currentY + noOutputs + 1.5, "connector");
	...
	if ((mSourceEntity.getClockPort() != -1) && (mSourceEntity.getResetPort() != -1))
	{
		...
		// clock port
		sWriter.customShape(currentX, currentY + entityHeight - 1, portWidth, 1, mSourceEntity.getPortName(clockPort), "port", "tlsport");
		sWriter.line(currentX - 1, currentY + entityHeight - 0.5, currentX, currentY + entityHeight - 0.5, "connector");
		sWriter.line(currentX, currentY + entityHeight - 0.35, currentX + 0.2, currentY + entityHeight - 0.5, "connector");
		sWriter.line(currentX, currentY + entityHeight - 0.65, currentX + 0.2, currentY + entityHeight - 0.5, "connector");
	...
	{
		sWriter.customShape(currentX, currentY + entityHeight, portWidth, 1, mSourceEntity.getPortName(clockPort), "port", "tlsport");
...
double FODG::calcTextWidth(int charCount, double fontSize){
	double yOffset = 0.52;
